...
public static class ServiceStopCommandAgent
{
    public static async Task<int> RunAsync(IWindowsServiceManager serviceManager, CancellationToken cancellationToken)
    {
        Console.WriteLine("[INFO] Stopping RenderByte Sync Windows Service...");

        if (!IsAdministrator() && Environment.GetEnvironmentVariable("RENDERBYTE_SYNC_TEST_MODE") != "1")
        {
            Console.Error.WriteLine("[ERROR] Administrator privileges are required to stop the Windows Service.");
            return 1;
        }
        ...
        try
        {
            ...
            Console.WriteLine("[OK] Service stopped successfully.");
            return 0;
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[ERROR] Failed to stop service: {ex.Message}");
            return 1;
        }
    }
